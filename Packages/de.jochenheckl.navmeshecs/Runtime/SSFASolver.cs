...
namespace de.JochenHeckl.NavMeshECS
{
    public static class SSFASolver
    {
        ...
        //    var funnelUp = CalculateFunnelUp( apex, funnelRight, funnelLeft );

        //    // for each portal after the first one which is the opening funnel...

        //    for (var portalIdx = 1; portalIdx < portals.Length; ++portalIdx)
        //    {
        //        var currentPortal = portals[portalIdx];

        //        if (!TestFunnel( apex, currentPortal.right, currentPortal.left, funnelUp ))
        //        {
        //            Swap( ref currentPortal.right, ref currentPortal.left );
        //        }

        //        // narrow the left side of the current funnel
        //        if (!TestFunnelLeft( apex, currentPortal.left, funnelLeft, funnelUp ))
        //        {
        //            PushPathVertex( ref pathVerticesContainer, ref currentPathVertexIndex, funnelLeft );

        //            apex = funnelLeft;

        //            if (TestDestination( apex, destination, portals.Skip( currentPathVertexIndex ) ))
        //            {
        //                PushPathVertex( ref pathVerticesContainer, ref currentPathVertexIndex, destination );
        //                return currentPathVertexIndex;
        //            }

        //            funnelLeft = currentPortal.left;
        //            funnelUp = CalculateFunnelUp( apex, funnelRight, funnelLeft );

        //            if (!TestFunnelRight( apex, funnelRight, currentPortal.right, funnelUp ))
        //            {
        //                PushPathVertex( ref pathVerticesContainer, ref currentPathVertexIndex, funnelRight );
        ...
        //            funnelRight = currentPortal.right;
        //            funnelUp = CalculateFunnelUp( apex, funnelRight, funnelLeft );
        //        }

        //        if (!TestFunnelLeft( apex, destination, funnelLeft, funnelUp ))
        //        {
        //            PushPathVertex( ref pathVerticesContainer, ref currentPathVertexIndex, funnelLeft );
        //        }

        //        if (!TestFunnelRight( apex, funnelRight, destination, funnelUp ))
        //        {
        //            PushPathVertex( ref pathVerticesContainer, ref currentPathVertexIndex, funnelRight );
        //        }
        ...
        public static int SolveV2( float3 startPosition, float3 destination, (float3 right, float3 left)[] portals, NativeArray<float3> pathVerticesContainer )
        {
            var currentPathVertexIndex = 0;
